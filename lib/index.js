...
async function _createVerifyData({
  ...
  if(verificationMethod) {
    ...
    keyAlgorithmName = verifier.algorithm.name;
  } else if(dataIntegrityProof.signer?.algorithm) {
    const signerAlgorithm = dataIntegrityProof.signer.algorithm;
    keyAlgorithmName = typeof signerAlgorithm === 'object' ?
      signerAlgorithm.name : signerAlgorithm;
  }
  if(!keyAlgorithmName) {
    throw new Error(
